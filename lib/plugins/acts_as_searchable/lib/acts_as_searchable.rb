      ...
      module InstanceMethods
        ...
        module ClassMethods
          ...
          def search(tokens, projects=nil, options={})
            ...
            find_options = {:include => searchable_options[:include]} unless searchable_options[:include].blank?
            find_options = {:joins => searchable_options[:joins]} unless searchable_options[:joins].blank?
            ...
            scope = self
            project_conditions = options[:project_conditions].blank? ? [] : options[:project_conditions]
            ...
            results = []
            results_count = 0
            projects_count = Hash.new { |p, c| p[c] = 0 }

            scope = scope.scoped({:conditions => project_conditions}).scoped(find_options)
            scope = scope.scoped({ :conditions => options[:extra_conditions] }) unless options[:extra_conditions].blank?
            results_count = scope.count(:all)
            scope.find(:all).group_by(&:project).each { |p, res| projects_count[p] = res.count }
            results = scope.find(:all, limit_options)

            [results, results_count, projects_count]
          end
        end
      end
    end
