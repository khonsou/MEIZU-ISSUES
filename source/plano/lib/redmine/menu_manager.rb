  ...
  module MenuManager
    ...
    module MenuHelper
      ...
      def render_main_menu(project)
        render_menu((project && !project.new_record?) ? :project_menu : :application_menu, project, 'nav nav-tabs')
      end
      ...
      def render_menu(menu, project=nil, css_class = 'nav')
        links = []
      ...
      def render_menu_node(node, project=nil)
        if node.children.present? || !node.child_menus.nil?
          ...
          return content_tag('li',
                               render_single_menu_node(node, caption, url, selected), class: selected ? 'active' : '')
        end
      end
