...
module IssuesHelper
  ...
  def show_detail(detail, no_html=false, options={})
    ...
    unless no_html
      ...
      if detail.property == 'attachment' && !value.blank? && atta = Attachment.find_by_id(detail.prop_key)
        ...
        value = link_to_attachment(atta, :download => true, target: '_blank')
    ...
    if detail.property == 'attr' && detail.prop_key == 'description'
      ...
      unless no_html
        diff_link = link_to 'diff',
          ...
          :title => l(:label_view_diff), remote: true
        s << " (#{ diff_link })"
      end
      s.html_safe
  ...
  def render_title_of_all_projects_issues
    ...
    unless params[:project_id] # this could be done in a better way %>
      s << '<div class="row">'
      s << "<div class=\"span24\" style=\"font-size:150%;padding-left:30px;padding-bottom:13px;\">#{l(:issues_of_all_projects)}<br/><br/></div>"
      s << '</div>'
    end
    s.html_safe
  end
